...
本章节提供对OM SDK的维护命令，具体说明请参见[表1](#命令说明table)。

**表 1**  命令说明<a id="命令说明table"></a>

|命令名称|命令功能|操作参考|
|--|--|--|
|manual_kmc_update.sh|预留接口|具体操作请参见[manual_kmc_update命令](#manual_kmc_update命令)。|
|update_alg.sh|预留接口|具体操作请参见[update_alg命令](#update_alg命令)。|
...
> - manual_kmc_update.sh具体功能已消减，属于预留接口，由用户自行实现底层kmc接口具体功能
...
2. 根据需要使用以下命令：
    ...
    - **./om\_ability\_policy.sh allow**  ***op***开启能力项。可选参数请参见[表1](#allow子命令参数说明)。
...
开源代码包含的公网地址请参考《[OM SDK 7.3.0 公网地址.xlsx](../resource/OM%20SDK%207.3.0%20公网地址.xlsx)》。
...
|用户名|描述|初始密码|密码修改方法|
|--|--|--|--|
|root|登录Atlas 200I A2 加速模块用户。|默认密码请参见“账户清单”中的“Atlas 200I A2 加速模块”章节获取。|以root用户执行passwd <username> 命令修改。|
|admin|登录边缘管理系统Web界面或命令行。|Edge@12#$|仅限于首次登录时使用该默认密码，成功登录后系统会要求强制修改默认密码。修改密码成功后使用修改后的密码登录。修改密码方式：使用admin用户登录，使用passwd命令修改密码。登录边缘管理系统Web界面，点击页面右上角“修改密码”，设置新的用户密码。|
