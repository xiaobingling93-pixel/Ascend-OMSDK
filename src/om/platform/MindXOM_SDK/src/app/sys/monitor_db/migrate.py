...
from monitor_db.register import register_models
...
def main(argv=None):
    ...
    if args.operate == MigrateOperate.INSTALL.value:
        return Migrate.execute_on_install(CommonConstants.MONITOR_EDGE_DB_FILE_PATH, INIT_COLUMNS)

    # 先尝试恢复数据库，数据库正常的情况下不会被恢复
    Restore(CommonConstants.MONITOR_BACKUP_DIR, CommonConstants.MONITOR_EDGE_DB_FILE_PATH).entry()
    return Migrate.execute(CommonConstants.MONITOR_EDGE_DB_FILE_PATH, INIT_COLUMNS)
